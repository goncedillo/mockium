...
class MockiumManager {
  ...
  set currentFeature(f) {
    this._currentFeature = f;
  }
  ...
  updateFeatures(features) {
    ...
    this._prompting.updateFeatures(features, logger.printFeaturesUpdated);

    this._ws.send(JSON.stringify({ type: ServerEvent.SERVER_MANAGER_UPDATED }));
  }
  ...
  goToSection(option) {
    ...
    if (option && option.go && typeof option.go === "function") {
      option.go();
    }
  }
